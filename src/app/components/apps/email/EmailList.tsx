      ...
      <SimpleBar className="max-h-[600px] h-[calc(100vh_-_100px)]">
        <div className="border-right border-color-divider h-full w-320">
          {filteredEmails.map((email) => (
            <div
              key={email.id}
              className={`cursor-pointer py-4 px-6 gap-3 items-center group bg-hover  ${selectedEmailId === email.id ? 'bg-lightgray dark:bg-lightprimary' : ''} `}
            >
              <div className="flex gap-3" onClick={() => { handleSelectEmail(email); openMail(true) }}>
                <Checkbox className="checkbox border border-border dark:border-darkborder h-[18px] w-[18px]"
                ...
                <div className="w-full">
                  ...
                  <div className="flex justify-between" >
                    ...
                    {email.label == "Promotional" ? (
                      <Badge color={"primary"} className="rounded-md">{email.label}</Badge>
                    ) : email.label == "Social" ? (
                      <Badge color={"error"} className="rounded-md">{email.label}</Badge>
                    ) : email.label == "Health" ? (
                      <Badge color={"success"} className="rounded-md">{email.label}</Badge>
